...
public class GameVM : ObservableObject {
    ...
    public init(with player1: PlayerVM, andWith player2: PlayerVM, rules: Rules, board: Board, isAR: Bool) throws {
        ...
        if isAR {
            self.arScene = GameARView(nbRows: rules.nbRows, nbColumns: rules.nbColumns, gameVM: self)
        } else {
    ...
    private func boardChanged(board: Board, cell: Cell?) {
        ...
        if let player, let cell {
            ...
            if isAR {
                arScene?.placeToken(row: cell.row, col: cell.col, owner: player)
            } else {
                if player == .player1 {
